...
for (dataset in dataset_names) {
  ...
  cluster_file <- file.path(dataset_dir, "qcd_labels.csv")
  ...
  png(file.path(dataset_dir, paste0(dataset, "_lineage_structure_qcd.png")),
  ...
  plot(
    ...
    xlab = "PC1", ylab = "PC2",
    main = paste0(dataset, " | lineage_structure |qcd ")
  )
  ...
  saveRDS(sce, file = file.path(dataset_dir, "slingshot_sce_qcd.rds"))
  cat("  已保存 SCE 对象和轨迹图：slingshot_sce_qcd.rds\n\n")
}
