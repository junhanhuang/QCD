...
for (dataset in dataset_names) {
  ...
  cluster_file <- file.path(dataset_dir, "spectral_labels_align.csv")
  ...
  sce <- SingleCellExperiment(
    assays = list(norm = t(log_mat))  # genes × cells
  )
  ...
  png(file.path(dataset_dir, paste0(dataset, "_lineage_structure_spectral.png")),
  ...
  plot(
    ...
    xlab = "PC1", ylab = "PC2",
    main = paste0(dataset, " | lineage_structure |spectral ")
  )
  ...
  saveRDS(sce, file = file.path(dataset_dir, "slingshot_sce_spectral.rds"))
  cat("  已保存 SCE 对象和轨迹图：slingshot_sce_spectral.rds\n\n")
}
